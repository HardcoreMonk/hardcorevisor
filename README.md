...
├───────┴─────────┴────────────────────────────────────┤
│  vmcore (Rust staticlib) — 69 FFI functions          │
│  ┌─────────┬──────────┬───────────┬───────────────┐ │
...
# Run tests (Rust 81 + Go 19 = 100 tests)
just test
...
|-----------|----------|---------|
| `vmcore/` | Rust | KVM core staticlib — 68 FFI, kvm_sys/kvm_boot/kvm_loader, io_uring, virtio-blk+io+net, TAP, ZFS/nftables driver |
| `hcvtui/` | Rust | Ratatui TUI — live dashboard, VM manager + create form, storage/network/HA views |
| `controller/` | Go | Orchestration — Compute (Dual VMM + etcd 영속화), Storage, Network, Peripheral, HA |
...
# Rust (81 tests) + Go (19 tests = 3 API + 16 E2E)
...
| **Monitoring** | 1 | `GET /metrics` (Prometheus) |
