...
│   ├── tests/
│   │   └── e2e_vm_lifecycle_test.go  # E2E 통합 테스트 42개
│   └── go.mod
...
just test             # 전체 테스트 (Rust 82 + Go 107 = 189)
...
# Go 전용
just go-test          # 전체 Go 테스트 (race detector, 107개)
just go-test-e2e      # E2E 통합 테스트만 (42개)
...
| E2E 통합 | `controller/tests/e2e_vm_lifecycle_test.go` | `go test -race ./tests/` | 42 |
| **합계** | | `just test` | **189** |
...
E2E 시나리오 (20개): VM 전체 생명주기, 잘못된 상태 전이(409), 백엔드 선택(rustvmm+qemu), QEMU 생명주기, 혼합 백엔드, 동시성(10개 병렬), 스텁 엔드포인트, 미들웨어(RequestID/CORS), 백업 생명주기, 스토리지 CRUD, 디바이스 Attach/Detach, 클러스터 운영(펜싱), VM 마이그레이션, 네트워크 방화벽, 스토리지 스냅샷, API 정보, 템플릿 생명주기, 페이지네이션, VM 스냅샷, 이미지 레지스트리.
