...
just test             # 전체 테스트 (Rust 80 + Go 15 = 95)
...
just rust-test-vmcore     # vmcore 테스트만 (80개)
...
```bash
just rust-test-vmcore                     # vmcore 전체 (80개, 직렬)
...
### Go Controller 테스트 (15개)

```bash
just go-test              # 전체 (race detector, 15개)
just go-test-api          # API 유닛 테스트만 (3개)
just go-test-e2e          # E2E 통합 테스트만 (12개)
...
| 범위 | 위치 | 실행 방법 | 테스트 수 |
|------|------|----------|----------|
| vmcore 유닛 | `vmcore/src/*.rs` | `cargo test -p vmcore -- --test-threads=1` | 80 |
...
| E2E 통합 | `controller/tests/e2e_vm_lifecycle_test.go` | `go test -race ./tests/` | 12 |
| **합계** | | `just test` | **95** |
...
E2E 시나리오 (12개): VM 전체 생명주기, 잘못된 상태 전이(409), 백엔드 선택(rustvmm+qemu), QEMU 생명주기, 혼합 백엔드, 동시성(10개 병렬), 스텁 엔드포인트, 미들웨어(RequestID/CORS), 백업 생명주기, 스토리지 CRUD, 디바이스 Attach/Detach, 클러스터 운영(펜싱).

## 아키텍처: 핵심 설계 패턴

### FFI 경계 (Rust → Go)

핵심 통합 패턴: `vmcore`가 C staticlib으로 컴파일되어 CGo를 통해 Go에 링크된다. 총 68개 FFI 함수.
...
### 스토리지/네트워크 드라이버 패턴 (Go)

플러그어블 드라이버 아키텍처:
- **`StorageDriver`** 인터페이스 → `MemoryDriver` (기본) / `ZFSDriver` (zpool/zfs CLI)
- **`NetworkDriver`** 인터페이스 → `MemoryDriver` (기본) / `NftablesDriver` (nft CLI)
- 설정: `HCV_STORAGE_DRIVER=zfs` 환경변수 또는 `hcv.yaml` → `storage.driver: zfs`
- ZFS: `zpool list`, `zfs create -V`, `zfs destroy`, `zfs snapshot`, parseSize()
- nftables: `inet hcv_filter` 테이블 자동 생성, `nft add rule` 실행 (best-effort, 실패 시 Enabled=false)
