...
│       ├── app.rs             # App 상태, tick() 폴링, VM 액션 핸들링
│       ├── api_client.rs      # REST API 클라이언트 (reqwest, 3초 타임아웃)
│       ├── keybindings.rs     # vim 스타일 키바인딩 + VM 액션 (s/x/p/d)
...
│   │   └── hcvctl/main.go     # CLI (Cobra, 버전 주입)
│   ├── internal/
│   │   ├── api/
...
│   │   ├── compute/
│   │   │   ├── compute.go        # ComputeService, RustVMMBackend, BackendSelector
...
just test             # 전체 테스트 (Rust 54 + Go 11 = 65)
...
just rust-test-vmcore     # vmcore 테스트만 (54개)
...
```bash
just rust-test-vmcore                     # vmcore 전체 (54개, 직렬)
...
| 범위 | 위치 | 실행 방법 | 테스트 수 |
|------|------|----------|----------|
| vmcore 유닛 | `vmcore/src/*.rs` | `cargo test -p vmcore -- --test-threads=1` | 54 |
...
핵심 통합 패턴: `vmcore`가 C staticlib으로 컴파일되어 CGo를 통해 Go에 링크된다. 총 51개 FFI 함수.
...
- Docker: 개발 서비스 실행용 (etcd :2379, Prometheus :9090, Grafana :3000, Controller :8080)
...
- CI: `ubuntu-24.04`, 대상 브랜치 `main`/`develop`, 파이프라인: lint → test → e2e → build → security
