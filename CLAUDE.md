...
just test             # 전체 테스트 (Rust 65 + Go 11 = 76)
...
just rust-test-vmcore     # vmcore 테스트만 (65개)
...
```bash
just rust-test-vmcore                     # vmcore 전체 (65개, 직렬)
...
| 범위 | 위치 | 실행 방법 | 테스트 수 |
|------|------|----------|----------|
| vmcore 유닛 | `vmcore/src/*.rs` | `cargo test -p vmcore -- --test-threads=1` | 65 |
...
핵심 통합 패턴: `vmcore`가 C staticlib으로 컴파일되어 CGo를 통해 Go에 링크된다. 총 55개 FFI 함수.
