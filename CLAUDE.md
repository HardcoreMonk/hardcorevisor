...
just test             # 전체 테스트 (Rust 82 + Go 20 = 102)
...
### Go Controller 테스트 (20개)

```bash
just go-test              # 전체 (race detector, 20개)
just go-test-api          # API 유닛 테스트만 (3개)
just go-test-e2e          # E2E 통합 테스트만 (17개)
...
| E2E 통합 | `controller/tests/e2e_vm_lifecycle_test.go` | `go test -race ./tests/` | 17 |
| **합계** | | `just test` | **102** |
...
E2E 시나리오 (17개): VM 전체 생명주기, 잘못된 상태 전이(409), 백엔드 선택(rustvmm+qemu), QEMU 생명주기, 혼합 백엔드, 동시성(10개 병렬), 스텁 엔드포인트, 미들웨어(RequestID/CORS), 백업 생명주기, 스토리지 CRUD, 디바이스 Attach/Detach, 클러스터 운영(펜싱), VM 마이그레이션, 네트워크 방화벽, 스토리지 스냅샷, API 정보, 템플릿 생명주기.

## 아키텍처: 핵심 설계 패턴

### FFI 경계 (Rust → Go)

핵심 통합 패턴: `vmcore`가 C staticlib으로 컴파일되어 CGo를 통해 Go에 링크된다. 총 69개 FFI 함수.
